...
procedure TranslacaoDePontos(const Coord:array of Tcoord; var fpxcg,fpycg:real;var CoordT:array of Tcoord);
procedure RotacaoDePontos(const Angulo:real;Coord:array of Tcoord; var CoordR:array of TCoord);
...
function TensaoArmAtiv(var varEpsilonpyd,varEpsilonpu,varfpyd,varfptd,varEpsilont:Real):real;
function ForcaConcreto(Poligono:array of Tcoord;ffcnfcd,ffcnxalfa,ffcnxEpsilonC2,ffcnNconcreto:real):real;
function MomentoXXConcreto(Poligono:array of Tcoord;ffcnfcd,ffcnxalfa,ffcnxEpsilonC2,ffcnNconcreto:real):real;
function MomentoYYConcreto(Poligono:array of Tcoord;ffcnfcd,ffcnxalfa,ffcnxEpsilonC2,ffcnNconcreto:real):real;
...
begin
  if y1<>y2 then // Todas as condições já excluem a chance de y1=y2, aqui é possível criar uma faixa de segurança, y1/y2>0,0001, por ex
     begin
       ffuncc:=((y1*x2-y2*x1)/(y1-y2));
       ffuncd:=((x1-x2)/(y1-y2));
     end;
  if y1=y2 then // Todas as condições já excluem a chance de y1=y2, mesmo assim, é melhor prevenir, criando uma certa seguranca y1/y2>0,0001, por ex
    begin
       ffuncc:=0;
       ffuncd:=0;
     end;
   if (((ymax-x)+xEpsilonC2-y)/xEpsilonC2)<=0 then ffunce:=0; // Isto é, logaritmo neperiano de um numero extremamente pequeno - mas maior e diferente de zero!
   if (((ymax-x)+xEpsilonC2-y)/xEpsilonC2)>0 then ffunce:=exp(ln(((ymax-x)+xEpsilonC2-y)/xEpsilonC2)*(ffcnNconcreto+1)); // Rodar a função!


  Result:=-0.85*fcd*(-(xEpsilonC2*ffunce*(ffuncc*(ffcnNconcreto+2)+ffuncd*((ymax-x)+xEpsilonC2+ffcnNconcreto*y+y)))
...
begin
     ...
     begin
         fpAc:=fpAc+Coord[fContador].xi*Coord[fContador+1].yi-Coord[fContador+1].xi*Coord[fContador].yi;
         fSxx:=fSxx+(Coord[fContador].xi*Coord[fContador+1].yi-Coord[fContador+1].xi*Coord[fContador].yi)*(Coord[fContador].yi+Coord[fContador+1].yi);
         fSyy:=fSyy+(Coord[fContador].xi*Coord[fContador+1].yi-Coord[fContador+1].xi*Coord[fContador].yi)*(Coord[fContador].xi+Coord[fContador+1].xi);
     end;
...
iContador:integer;
   ...
   begin
     for iContador:=0 to Length(Coord)-1 do
         begin
             CoordR[iContador].xi:=Coord[iContador].xi*cos(Angulo*pi/180)-Coord[iContador].yi*sin(Angulo*pi/180);
             CoordR[iContador].yi:=Coord[iContador].xi*sin(Angulo*pi/180)+Coord[iContador].yi*cos(Angulo*pi/180);
...
function TensaoArmPas(var varEpsilonS,varEs,varfyd:Real):real;
begin
     if varEpsilonS<=varfyd/varEs*1000 then  //Trabalhando apenas antes da ruptura por compressão
        begin
        TensaoArmPas:=-varfyd;
...
begin
  ...
  if varEpsilont<0 then //Lembrar: negativo e tração; e positivo compressao
    begin
      ...
      if varEpsilont<varEpsilonpyd then
        begin
          Result:=varfpyd+(varfptd-varfpyd)/(varEpsilonpu-varEpsilonpyd)*(varEpsilont-varEpsilonpyd);
        end;
    end;
end;
...
begin
  ...
  for fcnConcContador:=0 to Length(Poligono)-2 do // Inicio do calculo das forças no concreto
            begin
                if abs(Poligono[fcnConcContador].yi-Poligono[fcnConcContador+1].yi)>0.000001 then // Medida para evitar travar o programa com (Y1 = Y2) que acarreta ( Denominador = Zero )
                   begin

                      if Poligono[fcnConcContador].yi<Poligono[fcnConcContador+1].yi then // Primeiro ponto MENOR que segundo

                                begin
                                    if Poligono[fcnConcContador].yi<ffcnvmax-ffcnxalfa then //Primeiro Ponto Em tração
                                      begin
                                         ...
                                         if Poligono[fcnConcContador+1].yi>ffcnvmax-ffcnxalfa then  // Segundo ponto numa área comprimida
                                           begin
                                              if Poligono[fcnConcContador+1].yi<=ffcnvmax-ffcnxalfa+ffcnxEpsilonC2 then // Segundo ponto na denominada area 1
                                                begin
                                                  Result:=Result
                                        ...
                                        begin
                                          if Poligono[fcnConcContador].yi>=ffcnvmax-ffcnxalfa then
                                            begin
                                              ...
                                              if Poligono[fcnConcContador+1].yi>ffcnvmax-ffcnxalfa+ffcnxEpsilonC2 then // Segundo ponto na denominada area 2
                                                begin
                                                Result:=Result
                                                         +Ncd1(Poligono[fcnConcContador].xi,Poligono[fcnConcContador].yi,Poligono[fcnConcContador+1].xi,Poligono[fcnConcContador+1].yi,ffcnvmax,ffcnxalfa,ffcnvmax-ffcnxalfa+ffcnxEpsilonC2,ffcnfcd,ffcnxEpsilonC2,ffcnNconcreto)
                                                         -Ncd1(Poligono[fcnConcContador].xi,Poligono[fcnConcContador].yi,Poligono[fcnConcContador+1].xi,Poligono[fcnConcContador+1].yi,ffcnvmax,ffcnxalfa,Poligono[fcnConcContador].yi,ffcnfcd,ffcnxEpsilonC2,ffcnNconcreto)
                                ...
                                if Poligono[fcnConcContador].yi>=ffcnvmax-ffcnxalfa+ffcnxEpsilonC2 then//Primeiro Ponto na segunda área de compressão
                                  begin
                                                         Result:=Result
                                                         +Ncd2(Poligono[fcnConcContador].xi,Poligono[fcnConcContador].yi,Poligono[fcnConcContador+1].xi,Poligono[fcnConcContador+1].yi,Poligono[fcnConcContador+1].yi,ffcnfcd)
                             ...
                             if Poligono[fcnConcContador+1].yi<Poligono[fcnConcContador].yi then // Segundo ponto MENOR que primeiro ponto
                               begin
                                 ...
                                 if Poligono[fcnConcContador].yi<=ffcnvmax-ffcnxalfa then //Primeiro Ponto Em tração
                                  ...
                                  if Poligono[fcnConcContador].yi<=ffcnvmax-ffcnxalfa+ffcnxEpsilonC2 then //Primeiro Ponto na primeira área de compressão
                                    begin

                                       if Poligono[fcnConcContador].yi>ffcnvmax-ffcnxalfa then  //Primeiro ponto na primeira area de compressão - condição 2
                                         begin

                                           if Poligono[fcnConcContador+1].yi>ffcnvmax-ffcnxalfa then // Segundo ponto na denominada area 1
                                             begin
                                              Result:=Result
                                                       +Ncd1(Poligono[fcnConcContador].xi,Poligono[fcnConcContador].yi,Poligono[fcnConcContador+1].xi,Poligono[fcnConcContador+1].yi,ffcnvmax,ffcnxalfa,Poligono[fcnConcContador+1].yi,ffcnfcd,ffcnxEpsilonC2,ffcnNconcreto)
                                                       -Ncd1(Poligono[fcnConcContador].xi,Poligono[fcnConcContador].yi,Poligono[fcnConcContador+1].xi,Poligono[fcnConcContador+1].yi,ffcnvmax,ffcnxalfa,Poligono[fcnConcContador].yi,ffcnfcd,ffcnxEpsilonC2,ffcnNconcreto);
                                           ...
                                           if Poligono[fcnConcContador+1].yi<=ffcnvmax-ffcnxalfa then // Segundo ponto na area de tração
                                              begin
                                                Result:=Result
                                                              +Ncd1(Poligono[fcnConcContador].xi,Poligono[fcnConcContador].yi,Poligono[fcnConcContador+1].xi,Poligono[fcnConcContador+1].yi,ffcnvmax,ffcnxalfa,ffcnvmax-ffcnxalfa,ffcnfcd,ffcnxEpsilonC2,ffcnNconcreto)
                                                              -Ncd1(Poligono[fcnConcContador].xi,Poligono[fcnConcContador].yi,Poligono[fcnConcContador+1].xi,Poligono[fcnConcContador+1].yi,ffcnvmax,ffcnxalfa,Poligono[fcnConcContador].yi,ffcnfcd,ffcnxEpsilonC2,ffcnNconcreto);
                                  ...
                                  if Poligono[fcnConcContador].yi>ffcnvmax-ffcnxalfa+ffcnxEpsilonC2 then //Primeiro Ponto na segunda área de compressão
                                    begin
                                      ...
                                      if Poligono[fcnConcContador+1].yi<ffcnvmax-ffcnxalfa+ffcnxEpsilonC2 then //Segundo ponto na primeira área de compressão
                                        begin
                                          if Poligono[fcnConcContador+1].yi>=ffcnvmax-ffcnxalfa then //Segundo ponto na primeira área de compressão - condição 2
                                            begin
                                              Result:=Result
                                                         +Ncd1(Poligono[fcnConcContador].xi,Poligono[fcnConcContador].yi,Poligono[fcnConcContador+1].xi,Poligono[fcnConcContador+1].yi,ffcnvmax,ffcnxalfa,Poligono[fcnConcContador+1].yi,ffcnfcd,ffcnxEpsilonC2,ffcnNconcreto)
                                                         -Ncd1(Poligono[fcnConcContador].xi,Poligono[fcnConcContador].yi,Poligono[fcnConcContador+1].xi,Poligono[fcnConcContador+1].yi,ffcnvmax,ffcnxalfa,ffcnvmax-ffcnxalfa+ffcnxEpsilonC2,ffcnfcd,ffcnxEpsilonC2,ffcnNconcreto)
                                      ...
                                      if Poligono[fcnConcContador+1].yi<ffcnvmax-ffcnxalfa then // Segundo ponto na area tracionada
                                        begin
                                          Result:=Result
                                                         +Ncd1(Poligono[fcnConcContador].xi,Poligono[fcnConcContador].yi,Poligono[fcnConcContador+1].xi,Poligono[fcnConcContador+1].yi,ffcnvmax,ffcnxalfa,ffcnvmax-ffcnxalfa,ffcnfcd,ffcnxEpsilonC2,ffcnNconcreto)
                                                         -Ncd1(Poligono[fcnConcContador].xi,Poligono[fcnConcContador].yi,Poligono[fcnConcContador+1].xi,Poligono[fcnConcContador+1].yi,ffcnvmax,ffcnxalfa,ffcnvmax-ffcnxalfa+ffcnxEpsilonC2,ffcnfcd,ffcnxEpsilonC2,ffcnNconcreto)
                                 ...
                                 end; // Final dos calculos com segundo ponto menor que o primeiro para compressão do concreto
                   end;
...
begin
               ...
               for fcnConcContador:=0 to Length(Poligono)-2 do // Inicio do calculo do momento xx no concreto
                  begin
                      if abs(Poligono[fcnConcContador].yi-Poligono[fcnConcContador+1].yi)>0.000001 then // Medida para evitar travar o programa com (Y1 = Y2) que acarreta ( Denominador = Zero )
                        begin
                                              ...
                                              if Poligono[fcnConcContador+1].yi>ffcnvmax-ffcnxalfa+ffcnxEpsilonC2 then   // Segundo ponto na denominada area de compressão 2
                                                begin
                                                  Result:=Result
                                                          ...
                                                          -Mcd1xx(Poligono[fcnConcContador].xi,Poligono[fcnConcContador].yi,Poligono[fcnConcContador+1].xi,Poligono[fcnConcContador+1].yi,ffcnvmax,ffcnxalfa,ffcnvmax-ffcnxalfa,ffcnfcd,ffcnxEpsilonC2,ffcnNconcreto)
                                                          +Mcd2xx(Poligono[fcnConcContador].xi,Poligono[fcnConcContador].yi,Poligono[fcnConcContador+1].xi,Poligono[fcnConcContador+1].yi,Poligono[fcnConcContador+1].yi,ffcnfcd)
                                          ...
                                          if Poligono[fcnConcContador].yi>=ffcnvmax-ffcnxalfa then
                                            begin
                                              ...
                                              begin
                                                Result:=Result
                                                         +Mcd1xx(Poligono[fcnConcContador].xi,Poligono[fcnConcContador].yi,Poligono[fcnConcContador+1].xi,Poligono[fcnConcContador+1].yi,ffcnvmax,ffcnxalfa,Poligono[fcnConcContador+1].yi,ffcnfcd,ffcnxEpsilonC2,ffcnNconcreto)
                                                         -Mcd1xx(Poligono[fcnConcContador].xi,Poligono[fcnConcContador].yi,Poligono[fcnConcContador+1].xi,Poligono[fcnConcContador+1].yi,ffcnvmax,ffcnxalfa,Poligono[fcnConcContador].yi,ffcnfcd,ffcnxEpsilonC2,ffcnNconcreto);
                                              end;
...
begin
     ...
     PontosMaximoMinimo(Poligono,ffcnvmax,ffcnvmin);
     for fcnConcContador:=0 to Length(Poligono)-2 do // Inicio do calculo do momento yy no concreto
                begin
                                       ...
                                       if Poligono[fcnConcContador+1].yi>ffcnvmax-ffcnxalfa then  // Segundo ponto numa área comprimida
                                         begin
                                            if Poligono[fcnConcContador+1].yi<=ffcnvmax-ffcnxalfa+ffcnxEpsilonC2 then // Segundo ponto na denominada area 1
                                              begin
                                                Result:=Result
                                                        +Mcd1yy(Poligono[fcnConcContador].xi,Poligono[fcnConcContador].yi,Poligono[fcnConcContador+1].xi,Poligono[fcnConcContador+1].yi,ffcnvmax,ffcnxalfa,Poligono[fcnConcContador+1].yi,ffcnfcd,ffcnxEpsilonC2,ffcnNconcreto)
                                                        -Mcd1yy(Poligono[fcnConcContador].xi,Poligono[fcnConcContador].yi,Poligono[fcnConcContador+1].xi,Poligono[fcnConcContador+1].yi,ffcnvmax,ffcnxalfa,ffcnvmax-ffcnxalfa,ffcnfcd,ffcnxEpsilonC2,ffcnNconcreto);
                                              end;
                                            if Poligono[fcnConcContador+1].yi>ffcnvmax-ffcnxalfa+ffcnxEpsilonC2 then   // Segundo ponto na denominada area de compressão 2
                                              begin
                                                Result:=Result
